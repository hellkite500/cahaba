    ...
    def execute(self, modelstream, WBD8, HYDROID):
        try:
            ...
            if not HYDROID in modelstream.columns:
                ...
                stream_centroid = gpd.GeoDataFrame(
                    {"geometry": modelstream.geometry.centroid},
                    crs=modelstream.crs,
                    geometry="geometry",
                )
                stream_wbdjoin = gpd.sjoin(
                    stream_centroid, WBD8, how="left", op="within"
                )
                stream_wbdjoin = stream_wbdjoin.rename(
                    columns={"geometry": "centroid", "index_right": "HUC8id"}
                )
                modelstream = modelstream.join(stream_wbdjoin).drop(
                    columns=["centroid"]
                )

                modelstream["seqID"] = (
                    (modelstream.groupby("HUC8id").cumcount(ascending=True) + 1)
                    .astype("str")
                    .str.zfill(4)
                )
                modelstream = modelstream.loc[modelstream["HUC8id"].notna(), :]
                modelstream = modelstream.assign(HYDROID=lambda x: x.HUC8id + x.seqID)
                modelstream = modelstream.rename(
                    columns={"HYDROID": HYDROID}
                ).sort_values(HYDROID)
            ...
            if bOK == False:
                ...
                modelstream = modelstream.sort_values(
                    by=[HYDROID], ascending=True
                ).copy()

                xy_dict = {}
                bhasnullshape = False
                for rows in modelstream[
                    ["geometry", FN_FROMNODE, FN_TONODE]
                ].iterrows():
                    if rows[1][0]:
                        # From Node
            ...
            for urow in modelstream[
                [FN_NEXTDOWNID, FN_TONODE, FN_FROMNODE, HYDROID]
            ].iterrows():
                tonodecol = urow[1][1]
                ...
                try:
                    ...
                    pass
                modelstream.loc[
                    modelstream[HYDROID] == hydroIDcol, [FN_NEXTDOWNID]
                ] = nextdownIDcol
