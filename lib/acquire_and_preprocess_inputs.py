...
def subset_wbd_to_nwm_domain(wbd, nwm_file_to_use):
    ...
    intersecting_indices = [
        not (gp.read_file(nwm_file_to_use, mask=b).empty) for b in wbd.geometry
    ]
...
def pull_and_prepare_wbd(
    ...
    if not os.path.exists(multilayer_wbd_geopackage) or overwrite_wbd:
        ...
        wbd_hu8.to_file(
            multilayer_wbd_geopackage, driver="GPKG", layer="WBDHU8"
        )  # Save.
        wbd_hu8.HUC8.to_csv(
...
def build_huc_list_files(path_to_saved_data_parent_dir, wbd_directory):
    ...
    huc_gpkg_list = [
        "WBDHU6",
        "WBDHU8",
    ]  # The WBDHU4 are handled by the nhd_plus_vector_dir name.
    ...
    included_huc4_file = os.path.join(
        path_to_saved_data_parent_dir, "included_huc4.lst"
    )
    included_huc6_file = os.path.join(
        path_to_saved_data_parent_dir, "included_huc6.lst"
    )
    included_huc8_file = os.path.join(
        path_to_saved_data_parent_dir, "included_huc8.lst"
    )
