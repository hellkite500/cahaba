...
def subset_vector_layers(
    ...
    subset_nhd_headwaters_fileName=None,
    dissolveLinks=False,
):
    ...
    nhd_streams = nhd_streams.merge(
        nhd_streams_vaa[["FromNode", "ToNode", "NHDPlusID", "StreamOrde"]],
        on="NHDPlusID",
        how="inner",
    )
    del nhd_streams_vaa
    ...
    if not nwm_headwaters.empty:
        nwm_headwaters.to_file(
            subset_nwm_headwaters_fileName,
            driver=getDriver(subset_nwm_headwaters_fileName),
            index=False,
        )
    del nwm_headwaters
    ...
    if subset_nhd_headwaters_fileName is not None:
        ...
        nhd_headwater_points = gpd.GeoDataFrame(
            {"NHDPlusID": nhd_headwater_streams["NHDPlusID"], "geometry": hw_points},
            geometry="geometry",
            crs=projection,
        )

        nhd_headwater_points.to_file(
    ...
    print(
        "Identify NHD streams downstream of relevant NHD Headwater streams", flush=True
    )
    nhd_streams = nhd_streams.explode()
    ...
    while Q:
        ...
        try:
            downstream_ids = nhd_streams.loc[
                nhd_streams["FromNode"] == toNode, :
            ].index.tolist()
    ...
    if dissolveLinks:
        ...
        print(
            "Dissolving NHD reaches to Links (reaches constrained to stream intersections)",
            flush=True,
        )

        nhd_streams.set_index("NHDPlusID", inplace=True, drop=False)
        nhd_streams["before_confluence"] = nhd_streams.duplicated(
            subset="ToNode", keep=False
        )

        nhd_streams.loc[nhd_streams["is_nwm_headwater"], "linkNo"] = np.arange(
            1, nhd_streams["is_nwm_headwater"].sum() + 1
        )

        Q = deque(
            nhd_streams.loc[nhd_streams["is_nwm_headwater"], "NHDPlusID"].tolist()
        )
        visited = set()
        ...
        for q in Q:
            link_geometries[nhd_streams.loc[q, "linkNo"]] = [
                p for p in zip(*nhd_streams.loc[q, "geometry"].coords.xy)
            ][::-1]
        ...
        while Q:
            ...
            for i in downstream_ids:
                if i not in visited:
                    ...
                    if nhd_streams.loc[q, "before_confluence"] or (
                        numberOfDownstreamIDs > 1
                    ):
                        # do not dissolve
                        linkNo += 1
                        nhd_streams.loc[i, "linkNo"] = linkNo

                        next_stream_geometry = [
                            p for p in zip(*nhd_streams.loc[i, "geometry"].coords.xy)
                        ][::-1]
                        ...
                        link_geometries[nhd_streams.loc[i, "linkNo"]] = (
                            link_geometries[nhd_streams.loc[i, "linkNo"]]
                            + next_stream_geometry
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d",
        "--hucCode",
        help="DEM to use within project path",
        required=True,
        type=str,
    )
    parser.add_argument(
        "-w", "--nwm-streams", help="DEM to use within project path", required=True
    )
    parser.add_argument(
        "-s",
        "--nhd-streams",
    ...
    parser.add_argument(
        "-l", "--nwm-lakes", help="DEM to use within project path", required=True
    )
    parser.add_argument(
        "-m", "--nwm-catchments", help="DEM to use within project path", required=True
    )
    parser.add_argument(
        "-u", "--wbd", help="Basins polygons to use within project path", required=True
    )
    parser.add_argument(
        "-c",
        "--subset-streams",
